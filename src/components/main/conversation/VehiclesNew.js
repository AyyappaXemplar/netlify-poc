...
import VehicleForm from "../../../components/main/conversation/forms/VehicleForm";
...
class VehiclesNew extends React.Component {
  vehicle = {
    ...
    model: false,
    trim: false,
    coverages: groupedCoverages.GOOD,
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (vehicleAge > LIABILITY_AGE || vehicleAge < NEW_VEHICLE) {
      history.push("/conversation/quotes/vehicles");
    } else {
      const vehicleId = vehicles[vehicles.length - 1].id;
      history.push(`/conversation/vehicles/${vehicleId}/edit-coverages`);
    }
  }
