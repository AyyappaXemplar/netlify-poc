  ...
  return (
    <div className="react-dropdown-select-dropdown react-dropdown-select-dropdown-position-bottom">
      <div className="react-dropdown-select-no-data">
        {!!state.search.length ? (
          <span>
            Searching for <strong>{state.search}</strong>
          </span>
        ) : (
          state.search
        )}
...
function VehicleSearch({
  ...
  const setVehicleSearchOptions = (event) => {
    ...
    const min_query = searchByVin
      ? VehicleConstants.VIN_MIN_SEARCH_CHARS
      : VehicleConstants.MIN_SEARCH_CHARS;
    if (query.length < min_query) return;

    const searchParamName = searchByVin ? "vin" : "query";
    VehicleOptionsApi.search(query, searchParamName).then((response) => {
  ...
  return (
    <CustomSelect
      ...
      placeholder={
        searchByVin
          ? t("form.fields.vehicle.vinSearchPlaceholder")
          : t("form.fields.vehicle.searchPlaceholder")
      }
      options={options}
